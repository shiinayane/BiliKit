...
| 脚本 | 一句话 |
|---|---|
| 🧩 **BiliKit Core** | 一装到位的核心增强：**CDN 优选**（救海外卡顿）· **埋点/广告拦截**（省流量降开销）· **主题**跟随系统 · 评论显 **IP 属地** · 播放**不息屏**，外加一个**统一设置面板**。 |
...
一个 `.user.js`，页面世界注入（`@grant none`），涵盖五个模块 + 设置面板。左下角悬浮齿轮打开面板，按模块开关、逐项配置；深浅色跟随系统。
...
- **零依赖 · 零框架** — 纯手写 TypeScript + 原生 DOM，无 React / Vue / 任何运行时库，整个产物 gzip 仅 **~16KB**。虚拟化是自写的几十行几何计算，不背通用库的包袱。（对照 [Bilibili-Gate](https://github.com/magicdawn/bilibili-gate) 那类重型 React 首页，虚拟化一关就动辄上 GB 内存）
- **窗口化虚拟化** — 只渲染可视窗口 ± 缓冲的卡片、上下占位撑高，DOM 数量恒定；锚点补偿滚动位置，上下翻不抖。长时间刷也不涨内存。
- **卡片** — 封面骨架微光 + 加载完淡入、封面悬停**自动预览**（videoshot 雪碧图，带进度条）、播放/弹幕/时长、UP 头像与名字（点进空间）、推荐理由徽章。
