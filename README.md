...
| 脚本 | 一句话 |
|---|---|
| 🧩 **BiliKit Core** | 一装到位的核心增强：**CDN 优选**（救海外卡顿）· **主题**跟随系统 · 评论显 **IP 属地** · 播放**不息屏**，外加一个**统一设置面板**。 |
...
2. 从 **GreasyFork** 安装：搜索 [「BiliKit」](https://greasyfork.org/scripts?q=BiliKit)，装 **BiliKit Core**（想要 App 推荐流再加装 **BiliKit Feed**）。
3. 或自行构建：`npm install && npm run build`，从 `dist/` 里安装生成的 `.user.js`（构建产物不入库）。

---

## 🧩 BiliKit Core

一个 `.user.js`，页面世界注入（`@grant none`），涵盖四个模块 + 设置面板。左下角悬浮齿轮打开面板，按模块开关、逐项配置；深浅色跟随系统。
...
- **窗口化虚拟化** — 只渲染可视窗口 ± 缓冲的卡片、上下占位撑高，DOM 数量恒定；锚点补偿滚动位置，上下翻不抖。长时间刷也不吃内存（对照重型 React 首页动辄上 GB）。
