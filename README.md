...
| 脚本 | 一句话 |
|---|---|
| 🧩 **BiliKit Core** | 一装到位的核心增强：**CDN 优选**（救海外卡顿）· **埋点/广告拦截**（省流量降开销）· **免登录**看评论/动态/1080p · **主题**跟随系统 · 评论显 **IP 属地** · 播放**不息屏**，外加一个**统一设置面板**。 |
...
3. 或自行构建：`npm install && npm run build`，从 `dist/` 里安装生成的 `.user.js`。

---

## 🧩 BiliKit Core

一个 `.user.js`，页面世界注入（`@grant none`），涵盖六个模块 + 设置面板。左下角悬浮齿轮打开面板，按模块开关、逐项配置；深浅色跟随系统。
...
- **埋点拦截** — 页面世界 hook `fetch`/`XHR`/`sendBeacon`，拦掉行为遥测（`data.bilibili.com/log/web` 等）与广告投放（`cm.bilibili.com`）请求，命中即短路、不发起。实测首页空闲时约 **65% 的请求是埋点 / 广告**（`data.bilibili.com` 一家占 62%、约 4.7 次/秒），拦掉零功能损失、页面无感，省流量降开销。广告拦截可单独关，支持自定义黑名单。
...
- **打开方式**（面板可选）：**底部抽屉** / 新标签页 / 当前页。
  - **抽屉**：同源 iframe 内嵌播放，从底部上滑、顶部留缝放浮动按钮（新标签 / 关闭），**隐藏站内顶栏 + 去广告**、加载遮罩（封面模糊铺底）、悬停预连接提速、主题跟随宿主；**在顶部继续下拉即关**，或点缝 / `Esc`。
- **悬浮工具**：右下角刷新内容 / 返回顶部。
...
| 脚本 | 作用 |
|---|---|
| [**浮窗抽屉** · Float](scripts/float.user.js) | 全站点视频 → 页内抽屉播放（多形态、滑动关闭）。Feed 自带的抽屉只管首页卡片；本脚本覆盖全站视频链接。 |
| [**回程** · Way Back](scripts/way-back.user.js) | 站内跨视频历史压扁、左下角回退栈点击跳回续播、左滑原生关标签页。 |
...
> **迁移提示**：Core 已内置 **CDN 优选 / 主题同步 / 评论属地 / 防睡眠**——若你装过这四个的旧独立版，改装 Core 后可将它们卸载（脚本内有单例守卫，短期并存不会重复执行，但功能重复）。

## 与其它增强脚本共存

BiliKit Core 的 **CDN 优选 / 埋点拦截 / 免登录** 都在页面世界 hook `fetch`/`XHR`。别的脚本若也 hook 并**改写同一批接口**（`playurl` / `nav` / `reply`），多层叠加的先后顺序不确定 → 常表现为**「时好时坏」**。原则：**优先用 BiliKit 内置的等价功能**；确要共存，就关掉与对方重叠的那个 BiliKit 模块给它让路。

| 同类脚本 | 与 BiliKit 的关系 | 建议 |
|---|---|---|
| [**beefreely**](https://github.com/vruses/beefreely)（免登录看评论 / 1080p） | 功能重叠，且都抢改 `nav`/`reply`/`playurl` → 时好时坏 | 用 Core 的**免登录**替代并卸载它；若坚持同装，关掉 Core 的**埋点拦截 + CDN 优选**给它让路 |
